...
public interface IGrafoDirigido {
 ...
 etiquetas pasadas por parametro deben ser validas (o sea, los vértices origen y destino deben existir en el grafo).
    ...
    /**
     * Metodo encargado de insertar un vertice en el grafo.No pueden ingresarse vertices con la misma etiqueta.La etiqueta
 especificada como parametro debe ser valida.
   ...
   /**
    ...
    void camino(TVertice origen, TVertice destino, TCaminos caminos, TCamino elCamino);
    
    void ordenTopologico(Comparable etiquetaInicio);
    
    LinkedList<TVertice> ordenParcial();
    
    TGrafoDirigido invertirGrafo();
    
    boolean esConexo();
    
    boolean tieneCiclo();
    
    void clasificarArcos(Comparable verticeOrigen, LinkedList<TArista> arcosArbol, LinkedList<TArista> arcosRetroceso, LinkedList<TArista> arcosAvance, LinkedList<TArista> arcosCruzados);
}
